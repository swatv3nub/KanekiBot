...
from Kaneki import dispatcher, SUDO_USERS, SUPPORT_USERS, LOGGER
from Kaneki.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler

import Kaneki.modules.helper_funcs.fun_strings as fun
...
`/decide` can be also used with regex like: `Kaneki? <question>: randomly answer "Yes, No" etc.`

Some other regex filters are:
`me too` | `goodmorning` | `goodnight`.

This Bot will reply random strings accordingly when these words are used!
...
DECIDE_HANDLER = DisableAbleMessageHandler(
    Filters.regex(r"(?i)^Kaneki\?"), decide, friendly="decide"
)
